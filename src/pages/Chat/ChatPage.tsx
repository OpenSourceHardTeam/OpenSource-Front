...
import ChatRoomList, { ChatRoom } from '../../components/ChatRoomList/ChatRoomList';
...
const ChatPage: React.FC = () => {
  ...
  const [roomMessages, setRoomMessages] = useState<{[key: number]: Message[]}>({});
      ...
      if (roomId === 1) {
        data = [
          ...
          },
          {
            id: 4,
            sender: '작별레',
            message: '두 분 모두 좋은 관점이에요! 저는 조금 다른 측면에서, 소설이 복잡한 현실에서 살아남기 위해 다른 세계를 경험할 수 있게 해주는 "도피처" 역할도 한다고 생각합니다. 어떤 역할도 중요하지 않을까요?',
            time: '10:23',
            isMine: false,
            senderId: 'user-4'
          }
    ...
    <div css={styles.PageContainer}>
      <div css={styles.ContentContainer}>
              ...
              messages.map((msg) => (
                <div key={msg.id} css={styles.MessageGroup}>
                  <div css={styles.MessageHeader}>
                    <span css={msg.isMine ? styles.MessageSenderMine : styles.MessageSender}>
                      {msg.sender}
                    </span>
                    <span css={styles.MessageTime}>{msg.time}</span>
                  </div>
                  <div css={msg.isMine ? styles.MessageBubbleMine : styles.MessageBubble}>
                    {msg.message}
                  </div>
                </div>
              ))
        ...
        <div css={styles.SidebarRight}>
          <Button  css={styles.LeaveButton} text={"채팅방 나가기"} type={'leave'}/>
             ...
             <InfoBoxWithTimers
              ...
              <div css={styles.InfoBox}>
                ...
                <div css={styles.InfoQuestion}>
                  <p css={styles.QuestionText}>디지털 시대에 종이책의 가치는?</p>
                  <p css={styles.QuestionDetail}>참여: 10명</p>
                </div>
                <div css={styles.InfoQuestion}>
                  <p css={styles.QuestionText}>작가의 의도는 얼마나 중요한가?</p>
